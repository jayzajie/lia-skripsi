...
@section('title', 'Tambah Jadwal Pembelajaran - Super Admin')

@section('content')
    <div class="position-relative">
        <h1 class="welcome-header">Tambah Jadwal Pembelajaran</h1>
    </div>

    <div class="dashboard-card">
        <div class="card-header d-flex justify-content-between align-items-center">
            <span>Form Tambah Jadwal</span>
            ...
            <form action="{{ route('superadmin.schedules.store') }}" method="POST">
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <select name="class_level" id="class_level" class="form-select @error('class_level') is-invalid @enderror" required>
                            ...
                            <option value="1" {{ old('class_level') == '1' ? 'selected' : '' }}>Kelas 1</option>
                            <option value="2" {{ old('class_level') == '2' ? 'selected' : '' }}>Kelas 2</option>
                            <option value="3" {{ old('class_level') == '3' ? 'selected' : '' }}>Kelas 3</option>
                            <option value="4" {{ old('class_level') == '4' ? 'selected' : '' }}>Kelas 4</option>
                            <option value="5" {{ old('class_level') == '5' ? 'selected' : '' }}>Kelas 5</option>
                            <option value="6" {{ old('class_level') == '6' ? 'selected' : '' }}>Kelas 6</option>
                        </select>
                        @error('class_level')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="room" id="room" class="form-control @error('room') is-invalid @enderror" value="{{ old('room') }}" placeholder="Contoh: Ruang 1A">
                        @error('room')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
                ...
                <div class="row mb-3">
                    <div class="col-md-6">
                        ...
                        <input type="text" name="academic_year" id="academic_year" class="form-control @error('academic_year') is-invalid @enderror" value="{{ old('academic_year', '2023/2024') }}" placeholder="Contoh: 2023/2024" required>
                        @error('academic_year')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
                    </div>
